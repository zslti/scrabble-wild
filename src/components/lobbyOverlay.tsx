...
import { getStartingTileMatrix } from './tile';
    ...
    return (
        <div className="overlay" onClick={(e)=>{
            ...
            <div className='overlay-panel'>
                ...
                {props.requiresLobbyName && <Input value={lobbyName} onChange={(value) => {setLobbyName(value); clearError('lobbyName');}} placeholder='Lobby name' icon='fa-solid fa-bars-staggered'/>}
                <AnimatedAppear isVisible={(errors.has("lobbyName") && errors.get('lobbyName')?.visible) ?? false} height='1.5rem' childComponent={<div className="overlay-subtext small-margin error">{errors.get('lobbyName')?.message}</div>}/>
                {props.requiresPassword && <AnimatedAppear isVisible={!isPublic} height='3rem' childComponent={<Input value={password} onChange={(value) => {setPassword(value); clearError('password');}} placeholder='Password' icon='fa-solid fa-lock' type='password'/>}/>}
                {props.requiresPassword && <AnimatedAppear isVisible={(errors.has("password") && errors.get('password')?.visible && !isPublic) ?? false} height='1.5rem' childComponent={<div className="overlay-subtext no-margin error">{errors.get('password')?.message}</div>}/>}
                ...
                <AnimatedAppear isVisible={(errors.has("name") && errors.get('name')?.visible) ?? false} height='1.5rem' childComponent={<div className="overlay-subtext small-margin error">{errors.get('name')?.message}</div>}/>
                {props.requiresPassword && <AnimatedAppear isVisible={!props.isCreatingLobby} height='3rem' childComponent={<Input value={password} onChange={(value) => {setPassword(value); clearError('joinPassword');}} placeholder='Password' icon='fa-solid fa-lock' type='password'/>}/>}
                {props.requiresPassword && <AnimatedAppear isVisible={(errors.has("joinPassword") && errors.get('joinPassword')?.visible) ?? false} height='1.25rem' childComponent={<div className="overlay-subtext no-margin error">{errors.get('joinPassword')?.message}</div>}/>}
                <AnimatedAppear isVisible={(errors.has("general") && errors.get('general')?.visible) ?? false} height='1.25rem' childComponent={<div className="overlay-subtext small-margin error">{errors.get('general')?.message}</div>}/>
                <div className="overlay-button-container">
                    <Button text={props.cancelText ?? 'Cancel'} className={`light-background overlay-button negative ${props.isMessage?'center':''}`} icon='fa-solid fa-xmark' onClick={() => {!props.isNonCancellable ? removeOverlay() : window.location.href = '/scrabblewild';}}/>
                    {!props.isMessage && <Button text={props.proceedText ?? ''} className='light-background overlay-button' icon={props.customProceedIcon == null?(props.isCreatingLobby?'fa-solid fa-plus':'fa-solid fa-arrow-right-to-bracket'):props.customProceedIcon} onClick={onProceed} isLoadingOnClick={props.isProceedButtonLoadingOnClick}/>}
                </div>
            </div>
        </div>
    )
