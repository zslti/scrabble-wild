...
function Player(props: Props) {
    ...
    return (
        <div className={"player " + (props.isAdmin?"white ":"") + props.className}>
            ...
            {props.isKickable && <a className="kick-anchor-element" onClick={(e) => createOverlay({title: `Kick ${props.name}`, requiresLobbyName: false, requiresPassword: false, isCreatingLobby: false, isNonCancellable: false, isConfirmMessage: true, message: `Are you sure you want to kick ${props.name}?`, cancelText: 'Cancel', customProceedIcon: "fa-solid fa-ban", proceedText: "Kick", customProceedFunction: kickFunction})}><i className="fa-solid fa-ban negative right clickable"></i></a>}
            {props.name}
