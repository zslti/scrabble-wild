    ...
    async function updateWordDefinition() {
        if(definition.size == 0){
            ...
            if(def){
                ...
                setDefinition(new Map([['error', 'The definition for this word is not available']]));
        }
    }
            ...
            <div className="definition-overlay-panel" style={{left: `calc(${props.position.x}px + 1rem)`, top: `calc(${props.position.y}px + 1rem)`}}>
                <div id='definition-height-helper' style={{position: 'absolute', opacity: '0', pointerEvents: 'none'}}>{
                    Array.from(definition).map((def, index) => {  
                        if(def[0] == 'error') return <div className="definition-overlay-definition" style={{color: "#a5a5a566", position: "absolute", left: "50%", transform: "translateX(-50%)", whiteSpace: "nowrap", paddingRight: "1.5rem", marginTop: "1rem"}}>{def[1]}</div>
                ...
                <AnimatedAppear isVisible={definition.size == 0} height='2rem' childComponent={<div className='center-items' style={{height: "2rem"}}><i className={`fa-solid fa-circle-notch no-margin white ${definition.size != 0?'hidden':''}`}></i></div>}/>
                <AnimatedAppear isVisible={definition.size != 0} height={isExpanded?`max(6rem, ${(document.getElementById("definition-height-helper")?.clientHeight ?? 0) + 20}px)`:'3rem'} childComponent={<div style={{position: 'relative', zIndex: '100000 !important'}}>{
                    Array.from(definition).map((def, index) => {  
                ...
                <div className="definition-button-container">
                    <div className={`definition-expand-button ${(isExpanded || definition.size <= 1)?'hidden':''}`} onClick={(e) => setIsExpanded(true)}>Show more</div>
                </div>
            </div>
        </div>
    )
